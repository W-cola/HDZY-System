...
function handleClick(item: NotificationItem) {
  if (item.link) navigateTo(item.link, item.query, item.state);
}
function navigateTo(
  link: string,
  ...
  else router.push({ path: link, query: query || {}, state });
}
watch(
  () => ({
...
<template>
  <BasicLayout
    ...
    @logout="handleLogout"
  >
    <template #logo-text>华档致远</template>
    <template #user-dropdown>
      <UserDropdown
        :avatar
        :menus
        :text="userStore.userInfo?.realName"
        description="北京华档致远科技有限公司"
        tag-text="华档"
        @clear-preferences-and-logout="handleLogout"
        @logout="handleLogout"
      />
